...
class BiasEncoding(nn.Module):
    ...
    def build(self, input_shape):
        ...
        # 创建可训练的权重
        self.sess_bias_embedding = nn.Parameter(
            torch.randn(self.sess_max_count, 1, self.embed_size) * 0.0001
        )
        self.embed_bias_embedding = nn.Parameter(
            torch.randn(1, 1, self.embed_size) * 0.0001
        )

    def forward(self, inputs):
        """
        inputs是个列表，代表每个会话的embedding矩阵
        """
        # 确保输入是列表
        ...
        # 初始化模型参数（如果还没有初始化）
        if not hasattr(self, 'embed_size'):
            self.build([x.shape for x in inputs])
            
        # 对每个会话加入偏置
        for i in range(len(inputs)):
            # 添加会话偏置和嵌入偏置
            inputs[i] = inputs[i] + self.sess_bias_embedding[i] + self.embed_bias_embedding
            
        return inputs

def sess_interest_division(embedding_layer_dict, user_behavior_input_dict, 
    ...
    for i in range(sess_max_count):
        ...
        if sess_name not in user_behavior_input_dict:
            raise ValueError(f"Session {sess_name} not found in input dict")
            
        # 获取会话输入
        sess_input = user_behavior_input_dict[sess_name]
        sess_embedding = []
        
        # 获取每个特征的embedding
        for feat in sess_feature_columns:
            if feat in sparse_feature_columns:
                sess_embedding.append(
                    embedding_layer_dict[feat](sess_input)
                )
        
        # 拼接所有特征的embedding
        if len(sess_embedding) > 1:
            sess_embedding = torch.cat(sess_embedding, dim=-1)
        else:
            sess_embedding = sess_embedding[0]
            
        tr_input.append(sess_embedding)
    
    # 添加偏置编码
    if bias_encoding:
        bias_encoding_layer = BiasEncoding(sess_max_count)
        tr_input = bias_encoding_layer(tr_input)
...
"""DSIN NetWork"""
def DSIN(feature_columns, sess_feature_list, sess_max_count=5, bias_encoding=True, singlehead_emb_size=8,
         att_head_nums=8, dnn_hidden_units=(200, 80), dnn_dropout=0.0):
    """
    建立DSIN网络
    ...
    hist_emb_size = sum(fc.embedding_dim for fc in feature_columns 
                       if not isinstance(fc, str) and fc.name in sess_feature_list)
